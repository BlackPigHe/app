...
public class mes_stu {
	...
	JButton jb_send,jb_back,jb_del;
	JPanel jp;
	JFrame jf;
	public mes_stu(final Student stu){
		...
		jt_name=new JTextField(stu.getName());
		jt_id=new JTextField(""+stu.getId());
		jt_sex=new JTextField(stu.getSex());
		jt_age=new JTextField(""+stu.getAge());
		jt_major=new JTextField(stu.getMajor());
		jt_school=new JTextField(stu.getSchool());
		jt_grade=new JTextField(stu.getGrade());
		jt_time=new JTextField(stu.getTime());
		...
		jb_send.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(ps.insertstu(stu)){
					lbl_msg.setText("更新成功");
				}else{
					lbl_msg.setText("更新失败，请重试！");
				}
		...
		jb_del.setBounds(170, 310, 80, 40);
